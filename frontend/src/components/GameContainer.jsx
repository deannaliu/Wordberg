...
import IceScramble from './IceScramble';
import { GameContext } from '../contexts/GameContext';

const GameContainer = ({ initialPlayers, setPlayers }) => {
  // Move useContext to the top level
  ...
  // Now we can use board directly from the context
  const isGameFinished = () => {
    return (
      initialPlayers.player1.selectedLetters.length === 7 &&
      initialPlayers.player2.selectedLetters.length === 7
    ) || (
      // Check if board is empty (all hexagons are null)
      board.every(letter => letter === null)
    );
  };
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="flex flex-col items-center relative z-10">
        {isGameFinished() ? (
          <IceScramble players={initialPlayers} />
        ) : (
          <>
            {/* Game Board */}
            <GameBoard 
